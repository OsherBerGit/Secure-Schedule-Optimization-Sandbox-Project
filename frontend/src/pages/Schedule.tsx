...
import { Play, Settings, Database, Calendar, List, LayoutGrid, LayoutDashboard, CheckCircle2, Sparkles, Loader2, Info, AlertCircle } from 'lucide-react';
        ...
        <div className="schedule-page">
            <div className="schedule-header">
                <div className="header-left">
                    <div className="header-icon-wrapper"><Calendar className="header-icon-purple" size={32} /></div>
                ...
                <div className="header-right">
                    <div className="view-switcher">
                        <button className={viewMode === 'gantt' ? 'active' : ''} onClick={() => setViewMode('gantt')}><LayoutGrid size={16} /> Gantt</button>
                        <button className={viewMode === 'table' ? 'active' : ''} onClick={() => setViewMode('table')}><List size={16} /> Table</button>
                    </div>
                    {!isWorker && canManage && (
                        <div className="algo-controls">
                            ...
                            <button className="icon-btn-settings" onClick={openConfigModal} disabled={strategy !== 'MEMETIC'}><Settings size={20} /></button>
            ...
            {(algoError || dataError || configError) && (
                <div className="error-banner">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ fontWeight: 600, fontSize: '1.05rem' }}>{algoError || dataError || configError}</span>
                        {validationErrors && validationErrors.length > 0 && (
                            <ul style={{ marginTop: '12px', paddingLeft: '24px', fontSize: '0.95rem', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                {validationErrors.map((err, idx) => <li key={idx}><strong>Violation:</strong> {err}</li>)}
                            </ul>
                        )}
                    </div>
                </div>
            ...
            {successMsg && (
                <div className="success-banner"><CheckCircle2 size={20} /><span>{successMsg.replace(/^[✅\s]+/, '')}</span></div>
            )}
            ...
            {!isGenerating && !scheduleResult && scheduledTasks.length === 0 && (
                <div className="empty-state-card">
                    <div className="empty-icon-wrapper"><Sparkles size={40} /></div>
                ...
                <div className="result-layout-vertical">
                    <div className="draft-preview-card">
                        ...
                        <div className="stats-grid">
                            <div className="stat-box"><span className="stat-label">Total</span><span className="stat-number">{scheduleResult.totalTasks}</span></div>
                            <div className="stat-box success"><span className="stat-label">Assigned</span><span className="stat-number">{scheduleResult.assignedTasks}</span></div>
                            <div className="stat-box warning"><span className="stat-label">Unassigned</span><span className="stat-number">{scheduleResult.unassignedTasks}</span></div>
                        </div>
            ...
            {isConfigModalOpen && <SchedulingConfigurationModal configs={configs} isLoading={isConfigLoading} error={configError} onClose={closeConfigModal} initialConfigId={selectedConfigId} onSelectConfig={selectConfig} onCreateConfig={createConfig} />}
        </div>
    );
};
