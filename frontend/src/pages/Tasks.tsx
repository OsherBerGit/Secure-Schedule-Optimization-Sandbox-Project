...
const Tasks: React.FC = () => {
        ...
        try {
            if (selectedTask?.id)
                await taskApi.update(
                    selectedTask.id,
                    taskData as import("../types").UpdateTaskRequest,
                );
            else
                await taskApi.create(
                    taskData as import("../types").CreateTaskRequest,
                );
            fetchTasks();
            handleModalClose();
        } catch (err: any) {
    ...
    const filteredTasks = useMemo(() => {
        return tasks.filter((t) => {
            ...
            const matchesStatus =
                !statusFilter || (t.taskStatusName || "") === statusFilter;
            const matchesPriority =
                !priorityFilter || (t.priorityName || "") === priorityFilter;
            const matchesSkill =
                !skillFilter ||
                t.requiredSkills?.some((s) => s.name === skillFilter);
            return (
                matchesSearch &&
                matchesStatus &&
                matchesPriority &&
                            ...
                            {filteredTasks.map((t) => {
                                ...
                                return (
                                    <tr key={t.id}>
                                        ...
                                        <td>
                                            <span
                                                className={`priority-badge priority-${priorityName.toLowerCase()}`}
                                            >
                                                {priorityName}
                                            </span>
                                        </td>
                                            ...
                                            <td className="actions-cell">
                                                <button
                                                    ...
                                                    title={
                                                        canEditTask
                                                            ? "Edit Task"
                                                            : "Cannot edit a scheduled or closed task"
                                                    }
                                                ...
                                                <button
                                                    className="btn-icon constraint-btn"
                                                    onClick={() =>
                                                        navigate(
                                                            `/task-constraints?taskId=${t.id}`,
                                                        )
                                                    }
