...
const UserModal = ({ user, onSubmit, onClose }: UserModalProps) => {
    ...
    const [role, setRole] = useState<"ADMIN" | "MANAGER" | "WORKER">(
        user?.role ?? "WORKER",
    );
    ...
    function handleChangeAvail(
        index: number,
        field: keyof AvailRow,
        value: string,
    ) {
        setAvailRows((prev) => {
            const n = [...prev];
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        for (let i = 0; i < availRows.length; i++) {
            for (let j = i + 1; j < availRows.length; j++) {
                ...
                if (
                    ...
                    return (
                        setErrorMsg(
                            `Overlapping shifts detected on ${r1.dayOfWeek}`,
                        ),
        ...
        try {
            ...
            if (
                ...
                for (const [f, m] of Object.entries(err.response.data))
                    if (typeof m === "string") errs[f] = m;
                if (Object.keys(errs).length > 0)
                    return (setFieldErrors(errs), setIsSubmitting(false));
                        ...
                        <div className="form-grid">
                            ...
                            >
                                ...
                                {availRows.length === 0 && (
                                    ...
                                    >
                                        No availability defined. Click "+ Add
                                        Shift" to set working hours.
                                    </div>
                                )}
                            </div>
                        </div>
