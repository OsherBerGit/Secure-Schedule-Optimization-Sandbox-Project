...
const SettlementModal = ({
    ...
    const filteredUsers = initialUsers.filter((w) => {
        const activeCount = settlements.filter(
            (s) => s.userId === w.id && s.statusName !== "COMPLETED",
        ).length;
        if (
            w.maxTasks !== null &&
            w.maxTasks !== undefined &&
            activeCount >= w.maxTasks
        )
            return false;
    ...
    const filteredTasks = initialTasks.filter((t) => {
        if (
            settlements.some(
                (s) => s.taskId === t.id && s.statusName !== "COMPLETED",
            ) ||
            (t.taskStatusName && t.taskStatusName !== "OPEN")
        )
            return false;
        ...
        <div className="modal-overlay">
            ...
            >
                <div className="modal-header">
                    <h2>
                        <CalendarDays size={22} className="text-primary" /> Add
                        Settlement
                    </h2>
                    <button
                        type="button"
