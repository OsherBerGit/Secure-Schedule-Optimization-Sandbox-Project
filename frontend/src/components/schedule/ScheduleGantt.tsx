...
const ScheduleGantt: React.FC<ScheduleGanttProps> = ({
    ...
    const { minDate, maxDate, totalMs, userSchedules, timelineTicks } =
        useMemo(() => {
            ...
            const schedules = users
                .map((w) => ({
                    user: w,
                    userTasks: scheduledTasks.filter(
                        (t) => assignmentMap.get(t.id) === w.id,
                    ),
                }))
            ...
            return {
                minDate: min,
                maxDate: max,
                totalMs: max.getTime() - min.getTime() || 1,
                        ...
                        {userSchedules.map((ws) => (
                            ...
                            >
                                <div className="gn-user-side-info">
                                    ...
                                    <div className="gn-user-task-count">
                                        {ws.userTasks.length} tasks
                                    </div>
                                </div>
                                ...
                                >
                                    ...
                                    {ws.userTasks.map((task) => {
                                        ...
                                        const endTs = task.deadline
                                            ? new Date(task.deadline).getTime()
                                            : startTs +
                                              (task.durationHours || 1) *
                                                  3600000;
                                        const leftPercent =
                                            ((startTs - minDate.getTime()) /
                                                totalMs) *
                                            100;
                                        const widthPercent = Math.max(
                                            ((endTs - startTs) / totalMs) * 100,
                                            2,
                                        );
