...
const TaskModal = ({
    ...
    const [departmentId, setDepartmentId] = useState<number | string>(
        task?.departmentName
            ? (departments.find((d) => d.name === task.departmentName)?.id ??
                  "")
            : "",
    );
    ...
    useEffect(() => {
        setRequiredSkills(
            task?.requiredSkills ? task.requiredSkills.map((s) => s.id) : [],
        );
    }, [task]);
    useEffect(() => {
        if (!task) {
                ...
                <form onSubmit={handleSubmit} className="task-modal-form">
                    <div className="task-modal-body">
                        ...
                        <div className="form-grid">
                            ...
                            <div
                                className="modern-form-group full-width"
                                    ...
                                    {skills.map((s) => (
                                        ...
                                        >
                                            <input
                                                type="checkbox"
                                                id={`s-${s.id}`}
                                                checked={requiredSkills.includes(
                                                    s.id,
                                                )}
                                                onChange={() =>
                                                    handleSkillChange(s.id)
                                                }
                                                style={{
                                                    width: "18px",
                                                    height: "18px",
