...
export const useScheduleAlgorithm = () => {
        ...
        try {
            const res =
                strategy === "MEMETIC" && configId
                    ? await scheduleApi.runWithConfig(
                          strategy,
                          configId,
                          departmentId,
                      )
    ...
    const saveSchedule = async (tasks: Task[]) => {
        ...
        try {
            const assignments: SaveTaskAssignment[] =
                scheduleResult.assignments.map((a) => {
                    const originalTask = tasks.find((t) => t.id === a.taskId);

                    if (!originalTask)
                        throw new Error(
                            `Task ID ${a.taskId} not found in local state.`,
                        );

                    if (
                        originalTask.version === undefined ||
                        originalTask.version === null
                    )
                        throw new Error(
                            `Integrity Error: Task ID ${a.taskId} is missing a version number required for saving.`,
                        );
            ...
            if (isAxiosError(err)) {
                if (
                    ...
                    setError(
                        "Batch validation failed. Please review the specific errors.",
                    );
                } else if (err.response?.status === 409)
                    setError(
                        "Data Conflict: Tasks were modified by another user. Please refresh and try again.",
                    );
